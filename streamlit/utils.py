...
def fetch_precipitation_forecast(latitude=18.5898, longitude=73.7997, start_date=None, end_date=None):
    """
    Fetch daily precipitation forecast data from Open-Meteo API for a given location and date range.

    Notes:
    - This uses the non-archive forecast endpoint, intended for future dates.
    ...
    try:
        ...
        precipitation = data["daily"]["precipitation_sum"]
        return pd.DataFrame(
            {"date": pd.to_datetime(dates), "precipitation_mm": precipitation}
        )
    except requests.exceptions.RequestException as e:
