...
def dataFilter(df, verbose: bool = True):
    write = st.write if verbose else (lambda *a, **k: None)

    # 1. Filter to weekdays only for modeling
    df_ml = df[df['is_weekend'] == 0].copy()
    write(f"After filtering weekdays: {len(df_ml)} records")
    ...
    write(f"After get_dummies: {df_final.shape}")
    ...
    write(f"Feature matrix shape: {X.shape}")
    write(f"Target vector shape: {Y.shape}")
